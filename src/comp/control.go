...
var (
	control map[instruction.Type]map[step]uint64 = map[instruction.Type]map[step]uint64{
		...
		instruction.INST_MOV_REG_MEM_OFFSET: {
			0: MI_CMD_FETCH_INST,    // fetch next instruction
			1: MI_CMD_FETCH_OPERAND, // load operation register
			2: MI_PC_OUT | MI_IO_READ | MI_REG_OPERAND_1_OUT_X | MI_ALU_ENABLE | MI_ALU_ADD | MI_ALU_STORE | MI_STEP_INC,
			3: MI_ALU_OUT | MI_MAR_L_IN | MI_PC_INC | MI_STEP_INC,
			4: MI_PC_OUT | MI_IO_READ | MI_ALU_ENABLE | MI_ALU_ADC | MI_ALU_STORE | MI_STEP_INC,
