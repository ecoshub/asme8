...
const (
	MI_CMD_FETCH_INST    = MI_PC_OUT | MI_RAM_OUT | MI_INST_REG_IN | MI_PC_INC | MI_STEP_INC
	MI_CMD_FETCH_OPERAND = MI_PC_OUT | MI_RAM_OUT | MI_OPERAND_REG_IN | MI_PC_INC | MI_STEP_INC
)

var (
	control map[instruction.Type]map[step]uint64 = map[instruction.Type]map[step]uint64{
		instruction.INST_BREAK: {
			0: MI_PC_OUT | MI_RAM_OUT | MI_INST_REG_IN | MI_PC_INC | MI_STEP_INC,
			1: MI_BRK,
			2: MI_BRK,
			3: MI_BRK,
			4: MI_BRK,
		...
		instruction.INST_JMP_INM: {
			0: MI_PC_OUT | MI_RAM_OUT | MI_INST_REG_IN | MI_PC_INC | MI_STEP_INC, // fetch next instruction
